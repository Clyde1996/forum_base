<?php
    ...
    abstract class DAO{
        ...
        private static $dbname = 'forum_base';
         ...
         * pour pa faire chaque fois new pdo on fait un function qui a chaque fois a l'appelle de la function va se connecter a la base de donnes
         */
        ...
        {
            try{
                // Utilisation de la requête préparée pour sécuriser la requête SQL
                $stmt = self::$bdd->prepare($sql); // prepare et execute proteger de injection sql
                $stmt->execute($params);
              
                $results = ($multiple) ? $stmt->fetchAll() : $stmt->fetch(); // fetchAll() recupere tout les lignes 
                                                                        // fetch() recupere moi un ligne 
                $stmt->closeCursor();
